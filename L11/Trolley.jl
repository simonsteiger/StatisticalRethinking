...
# ╔═╡ 2dd9c6db-d5ff-4239-b64f-f70bead3daf4
begin
	import Downloads as DL
	using CSV, DataFrames, StatsPlots, Turing, StatsFuns
end

# ╔═╡ 39d05caa-4046-11ee-2b05-d7b47ff71601
begin
...
histogram(trolley.response)

# ╔═╡ 2a8976c0-024b-4109-aa6e-06cceec25e58
# Plot cumulative and incremental frequency
begin c = zeros(length(unique(trolley.response)))
	for i in sort(unique(trolley.response))
		c[i] = (i == 1 ? 0 : c[i-1]) + sum(trolley.response .== i)	
	end
	c0, c1 = [copy(c) for _ in 1:2]
	c0[1] = 0
	pop!(pushfirst!(c1, c1[1]))
	bar(c0, lw=1.25)
	bar!(c1, lw=1.25, legend=:none)
end

# ╔═╡ c564431b-77c6-4f3e-8269-5c12e2d0de8d
begin
	cum_prop = c ./  maximum(c)
	[cum_prop logit.(cum_prop)]
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "54343ff3ca0724c49b570d2e83e0a5bb7f354715"
