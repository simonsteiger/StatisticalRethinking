...
# ╔═╡ f542f439-c980-4e71-a255-ab6ab54e3f59
using CairoMakie

# ╔═╡ 1efab510-f737-4e60-8ee1-d14c4ae15bca
using Turing, Distributions, LinearAlgebra, Combinatorics

# ╔═╡ e3763883-74cb-499d-976d-18bf2f45a3e4
using DataFrames, Chain

# ╔═╡ 1e4b9c1d-e32b-4f27-92c7-c4b638ea6046
using LaTeXStrings

# ╔═╡ 393e04ec-9c02-4d93-883e-56af27bd01af
using PlutoUI

# ╔═╡ c871569a-88ab-11ef-0a8c-ad8226770fd7
DATAPATH = "https://raw.githubusercontent.com/rmcelreath/rethinking/master/data/"

# ╔═╡ ccbc0334-5277-4ee4-ab3d-92ad9d886673
md"""
# Synthetic data
"""

# ╔═╡ 991f9f39-7edc-47dd-9627-28f05ec96d72
md"""
## Code snippet 3.2
"""

# ╔═╡ fc316a21-ba0d-4bb9-a210-17d243838863
function sim_weight(H, β, σ)
	U = rand(Normal(0, σ), length(H))
	W = @. β * H + U
	return W
end

# ╔═╡ f74701a1-4022-4274-be51-be13f62faff5
begin
	n_slider = @bind N PlutoUI.Slider([10, 50, 100, 200], default=100)
	s_slider = @bind σ PlutoUI.Slider([1, 3, 5, 10], default=5)
	b_slider = @bind β PlutoUI.Slider([0.1, 0.3, 0.5, 0.9], default=0.5)
	md"""
	## Code snippet 3.3
	...
	Set the coefficient from height onto weight:
	$b_slider
	"""
end

# ╔═╡ e13d50eb-4c00-4c5f-9427-e2354c1c558c
md"""
N is currently $N.
...
# ╔═╡ e0627284-0a35-427c-b045-2e98021db8f1
min_height, max_height = 130, 170

# ╔═╡ fa7f0582-573f-47b0-82e7-2eaa1f6e6ead
H = rand(Uniform(min_height, max_height), N)

# ╔═╡ 7b58fc64-a1ba-465a-a69c-646d985ca7f5
W = sim_weight(H, β, σ)

# ╔═╡ 06510f88-b764-40ad-ba86-f7cbc966c4d4
let
	fig = Figure()
...
# ╔═╡ fedbd97e-708e-4672-b819-12c8a90c3a16
md"""
## Model notation
...
Note that we usually start the mathematical notation in the opposite order as we would code them in the model: first the likelihood, then the priors.
"""

# ╔═╡ 6c0934d9-9c0a-4cd7-bc49-2f1897d8aa9a
md"""
# Statistical models
...
# ╔═╡ 7b2aeb13-c65f-4e0b-8b75-70c3158f137d
...
# ╔═╡ 6a193031-83d8-4e74-89a5-349a8630d664
md"""
## Estimator
...
```math
\pi(\alpha, \beta, \sigma|H_i, W_i) = \frac{\pi(W_i|H_i, \alpha, \beta, \sigma) \pi(\alpha, \beta, \sigma)}{\text{Z}}
```

Z is not that important at the moment. It just makes sure that the left is a proper probability density function, i.e., integrates to 1.
"""

# ╔═╡ beada76d-1b43-4edf-84c8-5ddbd22e0099
md"""
## Grid approximate posterior

Grid approximate posterior is in the book, to be added here later
"""

# ╔═╡ a9aeb7f5-c844-4670-a7a2-e56adfda21f9
md"""
## Updating the posterior

The first (and currently only) plot shows sampling from the prior.

Click the button below to draw a new sample of intercepts and slopes.
"""

# ╔═╡ 000cee92-bf73-49dc-ba40-9cbcda7f121d
@bind go1 PlutoUI.Button("Resample")

# ╔═╡ cd56a5c9-5ddd-40d1-abbb-d44ef171d4ef
let go1
	p = MvNormal(zeros(2), I)
	...
	ax1 = Axis(fig[1, 1], limits=((-2, 2), (-2, 2)), xlabel="intercept", ylabel="slope")
	contour!(ax1, X, Y, Z, color=:black, levels=3)
	scatter!(ax1, samples, color=colors, marker=:x, markersize=15)
	
	ax2 = Axis(fig[1, 2], limits=((-2, 2), (-2, 2)), xlabel="x values", ylabel="y values")
	preds = [[α + β * x for x in -2:0.01:2] for (α, β) in eachslice(samples, dims=2)]
	for (y, c) in zip(preds, colors)
		lines!(ax2, -2:0.01:2, y, color=c)
	end
	
	fig
end

# ╔═╡ d9eb1833-a5b4-4e64-a52c-721f61b0cbf2
simulate(x; α=0, β=0.6, σ=1) = @. rand(Normal(α + β * x, σ))

# ╔═╡ fbd8e22b-df7e-4d7e-8e33-f214164e2aca
xs = randn(100)

# ╔═╡ a46282f4-5f3f-4b0d-a781-9561fa26fcc6
ys = simulate(xs)

# ╔═╡ 32e6e6da-0c32-4d8b-b92d-30adc967f666
@model function updating_posterior(x, y)
	α ~ Normal(0, 1)
	β ~ Normal(0, 1)
	σ ~ Exponential(1)
	μ = @. α + β * x
...
# ╔═╡ 0554bf7a-e799-4b1e-83d6-0b30d51e69c9
sdim(a) = x -> map(i -> i[a], x)

# ╔═╡ cf403826-bf3a-40b9-b8f6-be9ee40e03e3
md"""
Set the number of data points modelled: 

$(@bind n_input PlutoUI.Slider(0:100, default=5))
"""

# ╔═╡ 7402cf0a-4307-44da-bfd4-4a81f438d00e
fit_xs = collect(Iterators.take(xs, n_input))

# ╔═╡ 1f24ecaa-dbbb-48d7-954d-a5c25ed731df
fit_ys = collect(Iterators.take(ys, n_input))

# ╔═╡ b6943d87-7f86-4ef7-b384-becf9c5ee3d5
chn1 = sample(updating_posterior(fit_xs, fit_ys), NUTS(), 1000);

# ╔═╡ 30980347-3cb7-42f6-b85a-36636e717cdc
post_samples = DataFrame(chn1)[:, [:α, :β, :σ]];

# ╔═╡ 8186f09f-0fe8-45ae-8dff-c71c0ceba838
md"""
Current number of data points modelled: $n_input
"""

# ╔═╡ 3327d454-3506-4f90-a43c-777024e5149f
@bind go2 PlutoUI.Button("Resample lines")

# ╔═╡ 09c4af48-9c3c-426e-b19c-350505ef2c9d
sub_post_samples = let go2
	DataFrame(chn1)[rand(1:length(chn1), 5), [:α, :β, :σ]]
end;

# ╔═╡ 342883f7-4cac-495d-a7f7-77651d764680
let
	fig = Figure(size=(680,340))
	ax1 = Axis(fig[1, 1], limits=((-3, 3), (-3, 3)))
	ax2 = Axis(fig[1, 2], limits=((-2, 2), (-2, 2)))
	preds = [[α + β * x for x in -2:0.01:2] for (α, β, σ) in eachslice(Matrix(sub_post_samples), dims=1)]
	
	full_preds = [[α + β * x for x in -2:0.01:2] for (α, β, σ) in eachslice(Matrix(post_samples), dims=1)]
	M = reduce(hcat, full_preds)
	qs = [quantile(row, [0.025, 0.975]) for row in eachslice(M, dims=1)]
	
	band!(range(-2, 2, length=length(qs)), sdim(1)(qs), sdim(2)(qs), color=(:black, 0.15))
	lines!.(ax2, Ref(collect(-2:0.01:2)), preds)
	fig
end

# ╔═╡ ae5e7c30-5164-4790-a20b-a0cdd0a5fcda
md"""
!!! danger "Empty left panel?"
	I would like to visualize how slope and intercept are updated with the same plot as above. That probably requires modelling both as drawn from a two-dimensional multivariate normal.

	A task for the future! If you are interested in modelling correlated parameters, see [this section](https://storopoli.io/Bayesian-Julia/pages/11_multilevel_models/#correlated-random-intercept-slope_model) of Jose Storopoli's blog.
"""

# ╔═╡ 9105e819-da9f-446c-8673-abdc60f801a3
md"""
## Quadratic approximation

I will be skipping quadratic approximation and jump right to MCMC sampling to estimate the posterior. 

You will find more on quadratic approximation in McElreath's book.

In practice, MCMC is the work horse of posterior estimation. It will be introduced in a later lecture by McElreath and remains the standard for the rest of the lecture from that point on.
"""

# ╔═╡ ae092280-afa7-46ae-9394-3fd55a3a9255
@model function not_quap(H, W)
	α ~ Normal(0, 10)
	β ~ Uniform(0, 1)
	σ ~ Uniform(0, 10)
...
# ╔═╡ dc433d6d-ba03-43eb-8b48-a6039ae0c792
md"""
!!! note "Why MvNormal?"
	MvNormal means multivariate normal (distribution). These are normal distributions in more than one dimension: a 3D MvNormal looks a bit like a globular star cluster (and anything more is of course tough to imagine 🌝). 

	The `I` means that all of these distributions are uncorrelated, so you could also think of them simply as a stack (overly simplified, maybe).

	We would achieve the same if we iterated over each person `i` and sampled 
	```
	W[i] ~ Normal(μ[i], σ)
	```

	Instead, our likelihood function creates a vector `μ = @. α + β * H`, which we can then use to set up the "stacked" normal distributions. This is must faster to sample from for `Turing.jl`. In addition, it allows us to use the `predict` method for Turing models without having to make further adjustments to the model code.

	Note also that MvNormal expects a covariance matrix, so we want `σ^2` to be the variances on the main diagonal of that cov matrix.
"""

# ╔═╡ 9a569a73-9911-4218-a1c0-ea996a1b6c84
md"""
## Prior predictive simulation
"""

# ╔═╡ 5b19574b-c27e-41db-87b9-1249f18e629a
n_priorpred = 1000

# ╔═╡ e48a5756-3b69-4f67-bfe9-943f20eda43a
μ = let
	α = rand(Normal(0, 10), n_priorpred)
	β = rand(Uniform(0, 1), n_priorpred)
	
	[[a + b * x for x in 130:170] for (a, b) in zip(α, β)]
end

# ╔═╡ 1df1b847-ec8b-46f3-aada-6d9aa8a4c3d6
let
...
# ╔═╡ 9a3352de-023b-4f92-9e41-5fd999fb2a16
md"""
## Simulation-based validation

Let's return to the values we simulated with `sim_weights()` above.

Play with the sliders and check if the model captures the changed inputs!

Set sample size: $n_slider

Set standard deviation: $s_slider

Set coefficient of height on weight: $b_slider
"""

# ╔═╡ f6980aa2-d3c7-4a0f-913e-5c511e9ce67e
md"""
N is currently $N.
...
# ╔═╡ d505dfee-ec1e-4e29-b488-cdd797870e09
H

# ╔═╡ ae90029b-3f39-4b41-8137-7d78da9b1024
W

# ╔═╡ 5425ae0f-e52c-4180-bf71-2683ab4a1963
chn2 = sample(not_quap(H, W), NUTS(), 1000);

# ╔═╡ 356f5523-484b-418c-84bc-ae81f04eafc0
describe(chn2)

# ╔═╡ 1299e93a-c7a7-4f3d-a29b-45ced217a3a8
md"""
# Analysis

As McElreath says, once we have the simulation done, all that's left is to plug in the real data!
"""

# ╔═╡ 375f5b29-9d0a-4491-ab7a-ca4be769d947
chn3 = sample(not_quap(howell1.height, howell1.weight), NUTS(), 1000);

# ╔═╡ 166bbd1e-f828-4e73-bec0-5a400bba0759
describe(chn3)

# ╔═╡ 161cac1d-7c10-45b9-b064-32b5ca9f92a0
function pairsplot(chn::Chains, params)
	all_params = get_params(chn)
	a, b = [vec(all_params[param]) for param in params]
	divergent_idx = [round(Int, acc + 1.0) for acc in vec(all_params.is_accept)]
	colors = [(:red, 0.3), (:black, 0.3)]
	scatter(a, b, color=colors[divergent_idx])
end

# ╔═╡ 68d28f72-18d3-41d7-8a69-b14190ed940d
function pairsplot!(ax::Axis, chn::Chains, params)
	all_params = get_params(chn)
	a, b = [vec(all_params[param]) for param in params]
	divergent_idx = [round(Int, acc + 1.0) for acc in vec(all_params.is_accept)]
	colors = [(:red, 0.3), (:black, 0.3)]
	scatter!(ax, a, b, color=colors[divergent_idx])
end

# ╔═╡ a8c6aa5c-d746-4c40-9339-a7aa250145aa
md"""
Assembling the pairsplot that McElreath shows is a little laborious with our current setup but not a complex task.

(I didn't bother with the lower triangular of McElreath's plot matrix since it only contained numerical summaries.)
"""

# ╔═╡ 230dce01-330c-4e79-aee6-ce55f706a2d6
let
	params = get_params(chn3)
	fig = Figure(size=(500,500))
	ax1 = Axis(fig[1, 1], xlabel=L"α")
	ax2 = Axis(fig[1, 2], xlabel=L"α", ylabel=L"β")
	ax3 = Axis(fig[1, 3], xlabel=L"α", ylabel=L"σ")
	ax4 = Axis(fig[2, 2], xlabel=L"β")
	ax5 = Axis(fig[2, 3], xlabel=L"β", ylabel=L"σ")
	ax6 = Axis(fig[3, 3], xlabel=L"σ")
	hidedecorations!.([ax1, ax2, ax3, ax4, ax5, ax6], label=false)
	density!(ax1, vec(params.α), color=:grey80, strokecolor=:black, strokewidth=1)
	density!(ax4, vec(params.β), color=:grey80, strokecolor=:black, strokewidth=1)
	density!(ax6, vec(params.σ), color=:grey80, strokecolor=:black, strokewidth=1)
	pairsplot!(ax2, chn3, [:α, :β])
	pairsplot!(ax3, chn3, [:α, :σ])
	pairsplot!(ax5, chn3, [:β, :σ])
	fig
end

# ╔═╡ dcd1da72-ed4e-419f-ae88-328f72135323
md"""
## Posterior predictive distribution

!!! note "Prediction or retrodiction?"
	Note that Michael Betancourt prefers to call this a "retrodictive" check; we are not **PRE**dicting anything, just checking if the model describes the data it learned from well enough.
"""

# ╔═╡ 0fc5a65f-06f0-4cad-98a7-6107b666d61e
md"""
## Full of lines
"""
...
# ╔═╡ 9b619e17-960b-4bd9-97c3-d847d940b8bb
row_idxs = sample(1:nrow(howell1), n_input_analysis, replace=false)

# ╔═╡ 5d4860c5-9313-45a7-910c-cca7cc94f7de
sub_howell1 = howell1[row_idxs, :];

# ╔═╡ 0c168b70-da6a-4aea-88fd-6f246363f982
chn4 = sample(not_quap(sub_howell1.height, sub_howell1.weight), NUTS(), 1000);

# ╔═╡ 19cc5689-ccc0-49ec-9215-70e60ee8e9e5
let
	params = names(chn4, :parameters)
	xs = 130:180
	
	# Draw 50 samples from the posterior
	chain_df = DataFrame(chn4)[rand(1:length(chn4), 50), params]
	# Predict weight of these samples without uncertainty
	ys = [[a + b * x for x in xs] for (a, b) in zip(chain_df.α, chain_df.β)]
...
# ╔═╡ 98edc13d-1b2e-4236-aab7-c854b430f4e2
md"""
## Full of people

To see the full range of uncertainty contained in the model, we have to simulate from it for a sequence of new values.

Simulating from a posterior is easy in Turing. First, we need to tie the new data to the existing model structure. Note that the outcome $W$ is set to missing! Internally, this signals Turing's compiler that it should be sampled, not conditioned on!
"""

# ╔═╡ 10500c66-899e-4d85-9656-e88baf211c32
pred_model = let
	xs = 135:180
	pred_model = not_quap(xs, missing)
end

# ╔═╡ dee21227-e1a3-4613-8586-88c41fe2bfba
md"""
We can predict new data points by combining our existing posterior samples with the model we just defined:
"""

# ╔═╡ bf94d81e-9f75-43cb-ad45-6fd9a6970d54
predictions = predict(pred_model, chn4)

# ╔═╡ 2697ea98-5824-4225-89c6-57a5c68e227a
md"""
There's one column for each observation, while the number of rows matches the number of iterations in our chain. Great, but still a little unwieldy! Let's get it into shape and extract the quantiles for each observation.
"""

# ╔═╡ 8b2a3fe1-6714-49aa-96d0-0454638d181f
qs = @chain DataFrame(predict(pred_model, chn4)) begin
	select(_, names(_, Float64))
	Matrix(_)
	[quantile(slice, [0.05, 0.95]) for slice in eachslice(_, dims=2)]
end

# ╔═╡ d67be211-6ac3-41ff-baa1-1cf94e9b9005
md"""
Awesome! Let's plot!
"""

# ╔═╡ 4b746757-9419-4a20-b2e2-b10db3d76c5c
let
	xs = 135:180

	# Draw 50 samples from the posterior
	chain_df = DataFrame(chn4)[rand(1:length(chn4), 50), :]
	# Predict weight of these samples without uncertainty
	ys = [[a + b * x for x in xs] for (a, b) in zip(chain_df.α, chain_df.β)]
...
# ╔═╡ ea24c3e7-491e-4ca6-b68f-5d8f57ec5c0c
md"""
# Other helpers
"""

# ╔═╡ 72c0e62f-ca11-4342-8305-a9a361bccfdf
"Convert vector indexing to matrix indexing with `r` rows."
function midx(x, r)
    i = x % r == 0 ? r : x % r
    j = i == r ? x ÷ r : x ÷ r + 1
    return [j, i]
...
# ╔═╡ 72597591-e4ca-4749-99eb-7eff7ab3aae9
TableOfContents()
