...
# ╔═╡ 0a0f9924-3e99-11ee-3409-23634377f862
using Turing, StatsFuns, StatsPlots

# ╔═╡ 1505899a-897e-4506-8276-1a1f9a6aed84
# The idea here is that simpsons paradox is not a unitary phenomenon
# Any confounder can look like simpsons paradox
# Even non-confounder (here, group variable Z) can look like simpson

# ╔═╡ 3caf2979-c89b-4207-b41a-3833c4790ad9
begin
	N = 1000
	X = rand(Normal(), N)
	Z = rand(Bernoulli(), N)
	p = logistic.(X .+ ifelse.(Z .== 1, -1, 5))
	Y = rand.(Bernoulli.(p))
end

# ╔═╡ 9ca7b426-7471-4bf0-b414-360058d43972
@model function mX(X, Y)
	α ~ Normal()
	β ~ Normal()
...
# ╔═╡ 29b890a9-17c9-4c35-8ebf-d5ad15b969c7
c1 = sample(mX(X, Y), NUTS(), 1000)

# ╔═╡ 8400acee-dc83-480e-bfb8-2f201bb28f42
@model function mXZ(X, Z, Y)
	nZ = length(unique(Z))
	α ~ Normal()
	β ~ filldist(Normal(), nZ)

	for i in eachindex(X)
		v = logistic(α + β[Z[i]+1] * X[i])
		Y[i] ~ Bernoulli(v)
	end
end

# ╔═╡ 8eb69c73-ccfb-449d-828f-a7db0b3e8e04
c2 = sample(mXZ(X, Z, Y), NUTS(), 1000)

# ╔═╡ 02bd7bae-7e47-4979-b744-dffa9b56422a
pred(α, β, X) = logistic(α + β * X)

# ╔═╡ d7612cc9-f1ea-4752-b630-40fb1294bc60
Xrange = -2:0.1:2

# ╔═╡ 4a8fabbc-57e7-425a-bf1f-0f9c4ae32a2a
resX = 
	map(eachindex(c1["α"])) do i
		[pred(c1["α"][i], c1["β"][i], X) for X in Xrange]
	end

# ╔═╡ eaab8f8f-4e5b-4325-9094-b678da6529b8
plot(resX, color=1, alpha=0.03, label=false)

# ╔═╡ 59f3a6a6-bba7-4466-843d-a400e592d1c9
resXZ = 
	map(eachindex(c2["α"])) do i
		[pred(c2["α"][i], c2["β[$Z]"][i], X) for X in Xrange, Z in 1:2]
	end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "d38513ec65db3f2a6d1a81afe8c92bcd6c48f35d"
