...
# ╔═╡ 0b1a9512-5bc4-11ee-18d6-e79f2b7ca526
begin
...
# ╔═╡ a692540d-3db4-4c46-9dca-ecfd403ac258
...
begin
	...
	df_wafdiv = CSV.read(download(remote * "WaffleDivorce.csv"), DataFrame)
end

# ╔═╡ a6e1417a-a14e-4cd8-850d-ab0021528fc3
D_obs = standardise(df_wafdiv.Divorce)
...
@model function luxury(A, M, D_std, D_obs)
	α ~ Normal(0, 0.2)
	βA ~ Normal(0, 0.5)
	βM ~ Normal(0, 0.5)
	σ ~ Exponential(1)
	...
	μ = α .+ βA * A + βM * M
	D_true ~ MvNormal(μ, σ^2 * I) # unobserved

	D_obs .~ Normal.(D_true, D_std) # observed
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "996026c23e8ff48f917978f98d87353a8e3200b8"
