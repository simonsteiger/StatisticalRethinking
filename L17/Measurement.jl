...
# ╔═╡ ed43b302-5bb9-11ee-1385-c93802a2c9cf
using Turing, LinearAlgebra, StatsPlots

# ╔═╡ 57539418-d121-4d3f-bc3a-f6a886561cdd
# Simulate 500 measurements
N = 500

# ╔═╡ d6dc2286-87a1-4adb-a220-071f9d010c29
P = rand(Normal(), N)

# ╔═╡ 3f4c2854-166e-463b-93ee-f83a0b1571ea
C = rand.(Normal.(0 * P)) # True effect of Parental income on Child income is 0

# ╔═╡ 1d3aad4a-53d9-47a8-9bba-035a6b6adcd8
Pstar = rand.(Normal.(0.8 * P + 0.2 * C))

# ╔═╡ 1069de89-a679-40f3-840e-84fa48d7b209
@model function mPC(P, C)
	α ~ Normal()
	β ~ Normal()
	σ ~ Exponential(1)
...
# ╔═╡ 9381ee59-40d6-4b5a-9518-07733f25642b
begin
	chain1 = sample(mPC(Pstar, C), NUTS(), 1000)
	describe(chain1)
end

# ╔═╡ 4fca2cd4-6f77-481c-8fb2-feb55a102f24
begin
	density(chain1[:, "β", :]) # All effects estimated to be positive, despite true 0
	vline!([0], lw=1.5, color="black", legend=false)
	xlabel!("Estimated effect of parent income on child income")
	ylabel!("Density")
end
# Recall bias creates an untrue correlation
# TLDR; there is no general effect of measurement error
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "aae812d5354c50bfe904b939289f3d21a446a13d"
