...
# ╔═╡ fafa4200-368d-4f5c-a5e9-34e6cc01cade
begin
	using CSV, DataFrames, Turing, StatsPlots, StatsFuns, Random
	import Downloads as DL
end

# ╔═╡ 6b26ca2a-2ed8-11ee-310c-8f89bb162ece
begin
	remote = "https://raw.githubusercontent.com/rmcelreath/rethinking/master/data/"
	cats = CSV.read(DL.download(string(remote, "AustinCats.csv")), DataFrame)
end

# ╔═╡ 6f101cfc-9548-4cc0-b8a7-6e030095d2cb
begin
	days = cats.days_to_event
	adopted = [x == "Adoption" ? 1 : 0 for x in cats.out_event]
	color_id = [x == "Black" ? 1 : 2 for x in cats.color]
end

# ╔═╡ 4b68ae8c-4c6d-4b88-a9f4-03752d70b258
begin
	struct ExponentialCCDF <: ContinuousUnivariateDistribution end
	Distributions.rand(rng::AbstractRNG, d::ExponentialCCDF) = rand(rng)
	Distributions.logpdf(d::ExponentialCCDF, x::Real) = zero(x)
	Distributions.minimum(d::ExponentialCCDF) = 0.
	Distributions.maximum(d::ExponentialCCDF) = Inf
end

# ╔═╡ 496a5d55-9986-4441-9554-f079cb26053a
@model function meow(days, adopted, color_id)
	n = length(unique(color_id))
	α ~ filldist(Normal(), n)
	...
	for i in 1:length(days)
		if adopted == 1
			...
			# days[i] ~ ExponentialCCDF(λ[i]) # implement ExponentialCCDF
		end
	end
end

# ╔═╡ 40befbad-9b4c-44ac-b6d4-2a7badf301eb
begin
	m = meow(days, adopted, color_id)
	# chn = sample(m, NUTS(), 100) # see above, need to implement ExponentialCCDF
end

# ╔═╡ 98e54279-db1d-4f85-9e10-255c3fd9a6d2
mode(Exponential(1))
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "62649cda46526593e11c008a71fc932626b3bcda"
