...
# ╔═╡ ccd4b1a2-4a57-11ee-1f0b-15e6980a362f
begin
	using CSV, DataFrames, Chain, StatsPlots
	using Turing, StatsFuns, Statistics, StatsBase
	import Downloads as DL
end

# ╔═╡ 4130b510-03ce-497d-9cb9-d3ea7cca9ce5
begin
	remote = "https://raw.githubusercontent.com/rmcelreath/rethinking/master/data/"
	reedfrogs = CSV.read(DL.download(remote * "reedfrogs.csv"), DataFrame)
end

# ╔═╡ 307a8db5-87f6-4fee-841a-0bbf856facb8
let df = reedfrogs
	@chain df begin
		sort(_, :size)
		scatter(_.propsurv, group=_.size, legendtitle="Tanks")
	end
	hline!([mean(df.propsurv)], linestyle=:dash, color="black", label=false)
end

# ╔═╡ a15f2fdb-1912-4d47-9cfd-c42b94618ed8
@model function intercept_only(S, D)
...
begin
	mio = intercept_only(reedfrogs.surv, reedfrogs.density)
	chain_io = sample(mio, NUTS(), MCMCThreads(), 1000, 3)
	describe(chain_io)
end

# ╔═╡ 070abfde-5f91-4ff8-b898-de7a7f7234bd
# TODO show that the posterior for sigma is right where PSIS cross-validation points
# Multilevel models regularise for free!

# ╔═╡ 39874a37-74fb-4b2a-878b-c381d8c6d578
let chn = chain_io, df = reedfrogs
	# Extract parameters for plotting from the samples
	params = @chain DataFrame(describe(chn)[2]) begin
		select(r"par|.5%$")
		subset(:parameters => ByRow(x -> contains(string.(x), r"α")))
		leftjoin(_, DataFrame(mean(group(chn, "α"))), on=:parameters)
		DataFrames.transform(:mean => (x -> convert(Vector{Float64}, x)) => identity)
		DataFrames.transform(_, names(_, Float64) .=> ByRow(logistic) => identity)
	end

	μ = params.mean
	Q = [abs.(μ.-params[:, "2.5%"]), abs.(μ.-params[:, "97.5%"])]
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "d23b308eabdb928c1cd69ec4ac2f6b66249ba41c"
