...
@model function model_QO(W, X, S)
	nQ = maximum(W) # should be ordinal rank always, so maximum is OK?
	nO = maximum(X)
...
function do_O(df, chn, X)
	...
	# Create "observed" values, length matches chain length
	n = length(Q)
	W = rand(df.W, n)
	J = rand(df.J, n)
	X = fill(X, n)
...
# This takes around 10s on my machine
# The bootstraps introduce considerable variance in the models
let df = DataFrame([W, J], [:W, :J])
	contrasts = 
		map(1:20) do _
			O1 = do_O(df, chn_QOJ, 1)
			O2 = do_O(df, chn_QOJ, 2)
			O2.S .- O1.S 
			# 2 is French, 1 is American => positive difference means french is better
		end
	density(contrasts, color=1, alpha=0.3, legend=:none)
end
