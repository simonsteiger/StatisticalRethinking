...
# ╔═╡ a0dd9911-3c40-483e-a470-54583d6c1af9
using Distributions, DataFrames, Turing, StatsPlots

# ╔═╡ d9e2f35c-24bb-11ee-0dbd-41b331fc4fe0
# Exploring pipe confounds with simulated data about plant growth
# We have height of plant at time 0 (H0) and are intersted about height at time 1 (H1)
# There's also a fungus (F) around, reducing some of the plant growth!
# H0 --> H1 <-- F

# To take care of that, we've got an anti-fungal treatment (T)
# This affects F and H1 (effect on H1 unclear, may have a direct negative effect)
# H0 --> H1 <-- F
#          <-T->

# Do the benefits of T (kill fungus) outweigh the costs (toxicity to plant)?
# => Estimand: total causal effect of T

# Should we add F to our model?
# No! It would block the path from T to H1, since it is a pipe (mediator)
# In other words, we'd ignore the positive effect of T on H1 (kill F)
# We must include this effect to estimate the overall cost vs benefit ratio

# ╔═╡ 7f107718-5d2e-43ec-9878-4e5842f61f08
function sim_fungus()
	# number of plants
	N = 100
	...
	treatment = reduce(vcat, fill([0, 1], N ÷ 2))
	fungus = rand.(Binomial.(1, 0.5 .- treatment .* 0.4))
	H1 = H0 .+ rand.(Normal.(5 .- 3 .* fungus))

	# Make it a data frame
	df_pipe = DataFrame([H0, H1, treatment, fungus], [:H0, :H1, :treatment, :fungus])

	return df_pipe
end

# ╔═╡ 6a9078a4-9204-4d1d-ba10-624ca2d760b6
df_fungus = sim_fungus()

# ╔═╡ df6f2c0f-246a-44b2-856b-f29384702aba
@model function model_direct(H0, treatment, H1)
	α ~ Normal(0, 100)
	β_H0 ~ Normal(0, 10)
	β_trt ~ Normal(0, 10)
	σ ~ Uniform(0, 10)
	μ = α .+ β_H0 .* H0 + β_trt .* treatment

	return H1 ~ MvNormal(μ, σ^2)
end

# ╔═╡ d9419d13-8488-4c69-827f-a07798008385
function sample_direct(df)
	m = model_direct(df.H0, df.treatment, df.H1)
	chn = sample(m, NUTS(), MCMCThreads(), 1000, 3)
	return chn
end

# ╔═╡ 9c010848-b114-4e7c-9067-dffffc3ffb7c
begin
	chn_direct = sample_direct(df_fungus)
	describe(chn_direct)
end
# Treatment matters a great deal!
# This effect goes through reducing fungal growth

# ╔═╡ dd4b4d69-ff26-4dd8-a13b-f27a3023fb19
# We'll now specify the model including the entire pipe
# This means that we're answering the question:
# "Knowing if a plant had the fungus, what is the effect of treatment on growth?"
@model function model_pipe(H0, treatment, fungus, H1)
	α ~ Normal(0, 100)
	β_H0 ~ Normal(0, 10)
	β_trt ~ Normal(0, 10)
	β_fng ~ Normal(0, 10)
	σ ~ Uniform(0, 10)
	μ = α .+ β_H0 .* H0 + β_trt .* treatment + β_fng .* fungus

	return H1 ~ MvNormal(μ, σ^2)
end

# ╔═╡ 4c19c3ab-438b-4295-8663-2ee21997937d
function sample_pipe(df)
	m = model_pipe(df.H0, df.treatment, df.fungus, df.H1)
	chn = sample(m, NUTS(), MCMCThreads(), 1000, 3)
	return chn
end

# ╔═╡ 6371fae0-ef5a-4f40-b748-e0010b210325
begin
	chn_pipe = sample_pipe(df_fungus)
	describe(chn_pipe)
end
# The treatment has no direct impact on growth
# It only influences the fungus, which in turn influences growth
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "203e76083ff78f30e161c1a0c415474dda0aac63"
