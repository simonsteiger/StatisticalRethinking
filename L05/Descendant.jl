...
# ╔═╡ f8841f2e-2737-11ee-19db-918ea3cbe998
using Distributions, DataFrames, StatsPlots

# ╔═╡ 2c132722-d035-4f97-aa74-bc69895bc99d
function sim_descendant()
	N = 1000
	...
	Y = rand.(Bernoulli.(pY))
	A = rand.(Bernoulli.(pY))
	df = DataFrame([X, Y, Z, A], [:X, :Y, :Z, :A])
	return df
end

# ╔═╡ 95320fc9-2365-4712-840f-f973588b9b6d
df_desc = sim_descendant()

# ╔═╡ 3dffcb36-b600-4fa7-a00b-a2453c969815
let X = df_desc.X, Y = df_desc.Y
	cor(X, Y)
end

# ╔═╡ aedd14ae-f923-47fc-9e65-f51929817d82
let X = df_desc.X, Y = df_desc.Y, A = df_desc.A
	[cor(X[A .== i], Y[A .== i]) for i in [0, 1]]
end
# much weaker correlation than above

# ╔═╡ 85a4d597-0ce9-46c7-9074-a489e2516296
test = map(1:1000) do _
	sim_descendant()
end

# ╔═╡ 7895e972-a1cf-4c4e-b1e3-2d5c9efc0055
cors = [cor(test[i].X, test[i].Y) for i in eachindex(test)]

# ╔═╡ 4dccf257-30d4-4790-ac2b-de35542c62e7
density(cors)

# ╔═╡ caf0525c-e80e-4ae5-9d0b-d5e65d0f22dd
function err_cor(df)
	map(0:1) do j
		[cor(df[i].X[df[i].A .== j], df[i].Y[df[i].A .== j]) for i in eachindex(df)]
	end
end

# ╔═╡ 8b2439bb-e212-4c09-a59e-1685e2a7d74c
density(err_cor(test), label=["cor(X, Y)|A=0" "cor(X, Y)|A=1"])
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "38c579fe07f6467f109d2035e9c78a06b5f50f52"
