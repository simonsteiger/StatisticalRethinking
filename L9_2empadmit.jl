...
# ╔═╡ 544990df-b6b8-4ecb-b39e-14ec869b8396
begin
	remote = "https://raw.githubusercontent.com/rmcelreath/rethinking/master/data/"
	ucb = CSV.read(DL.download(string(remote, "UCBadmit_long.csv")), DataFrame)
end

# ╔═╡ 69b3939b-fe55-4d6a-956f-d7020655d473
ucb_agg = @chain ucb begin
	DataFrames.transform(:admit => (x -> (reject=x .== 0, admit=x .== 1)) => AsTable)
	select(Symbol("applicant.gender") => :gender, [:dept, :admit, :reject])
	groupby([:gender, :dept])
	combine([:admit, :reject] .=> sum .=> [:admits, :rejects])
	DataFrames.transform([:admits, :rejects] => ByRow((x, y) -> x + y) => :total)
end
...
begin
	chn_G = quicksample(model_G, G, A, N, 1)
	describe(chn_G)
end
