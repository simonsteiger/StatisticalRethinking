...
# ╔═╡ e8f87af4-27e7-11ee-3d6a-ff264af6f236
using Distributions, DataFrames, Turing, StatsPlots

# ╔═╡ 23699bd3-415d-48e9-aa65-fcd0d3f52045
function sim_backdoor()
	N = 200
	β_XY = 0
	β_UY = -1
	β_UZ = -1
	...
	U = rand(Bernoulli(0.5), N)
	Z = rand.(Normal.(β_UZ .* U, 1))
...
# ╔═╡ 4fd53416-6291-45a6-b175-f133aae2adb1
cor(df.X, df.Y)

# ╔═╡ efa81514-e760-4c4e-97ff-3adac7501563
@model function model_naive(X, Y)
	α ~ Normal(0, 1)
	β_XY ~ Normal(0, 1)
	σ ~ Exponential(1)
	μ = α .+ β_XY .* X

	return Y ~ MvNormal(μ, σ^2)
end

# ╔═╡ 4fd49c06-1909-4c14-a213-cf95a068fe78
# More general helper for sampling, maybe a good idea for Pluto? Undecided
...
# ╔═╡ fe0e515c-4b7d-4a5f-86cf-b6a1ffce487c
begin
	chn_naive = quicksample(model_naive, [df.X, df.Y])
	describe(chn_naive)
	# The pipe from U to X can flow, so there is a weak correlation between X and Y
end

# ╔═╡ 6914ee96-2732-4ca9-b786-69369ad9aa2d
@model function model_strat(X, Z, Y)
	α ~ Normal(0, 1)
	β_XY ~ Normal(0, 1)
	β_Z ~ Normal(0, 1)
	σ ~ Exponential(1)
...
# ╔═╡ 73cb3484-1cb2-4dd7-9cf5-63ad3406eabf
begin
	chn_strat = quicksample(model_strat, [df.X, df.Z, df.Y])
	describe(chn_strat)
	# Blocked the pipe between U and X with Z
	# Direct path X -> Y near 0
end

# ╔═╡ ff9cabe3-59a2-4a70-94b3-d065d781f0a5
function squash(x::AbstractArray)
    return reduce(hcat, x)'
end

# ╔═╡ f4c2619e-a58b-434c-b5c7-32127039af02
let
	p = get_params(chn_naive)
	q = get_params(chn_strat)
	density(squash(p.β_XY), label="Y|X")
	density!(squash(q.β_XY), label="Y|X,Z")
	title!("posterior distribution β_XY")
end

# ╔═╡ 9b09fd8b-6690-47e1-b545-2d2b35ca2452
function do_X_naive(df, chn; X=0)
	# Get posterior samples
	p = get_params(chn)
	α, β_XY, σ = [squash(p[i]) for i in [:α, :β_XY, :σ]]
	...
	Y = map(1:n) do i
		rand(Normal(α[i] + β_XY[i] * X[i], σ[i]))
	end

	return DataFrame([X, Y], [:X, :Y])
end

# ╔═╡ f9fd9021-d98f-4217-829b-cf4d7c17611f
function do_X_strat(df, chn; X=0, Z=0)
	# Get posterior samples
	p = get_params(chn)
	α, β_XY, β_Z, σ = [squash(p[i]) for i in [:α, :β_XY, :β_Z, :σ]]
	...
	# Simulate divorce values
	Y = map(1:n) do i
		rand(Normal(α[i] + β_XY[i] * X[i] + β_Z[i] * Z[i], σ[i]))
	end

	return DataFrame([X, Z, Y], [:X, :Z, :Y])
end

# ╔═╡ dd0c988d-3c34-4b82-bf1e-84567de1d09a
# do X = 12
let 
	p = density()
	map(1:30) do _
		df = do_X_naive(df, chn_naive; X=12)
		density!(df.Y, legend=:none, alpha=0.1, color=1)
		vline!([mean(df.Y)], color=2, alpha=0.05)
		title!("naive")
	end
	p
	...
	q = density()
	map(1:30) do _
		df = do_X_strat(df, chn_strat; X=12)
		density!(df.Y, legend=:none, alpha=0.1, color=1)
		vline!([mean(df.Y)], color=2, alpha=0.05)
		title!("stratified")
	end
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "b66e13a6197bad7a0cc8e9132aeb0833deb033ac"
...
git-tree-sha1 = "ac7f720e5d1902f2cd733992896c7fefaa163557"
